...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        print("[][][] [resnet_fcn.py] num_classes for ResNet: {}".format(num_classes))
...
def resnet101(pretrained=False, **kwargs):
    ...
    Args:
        pretraine (bool): If True, returns a model pre-trained on ImageNet
    """
...
class Resnet34_8s(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.out_middle:
            return logits_upsampled, y
        return logits_upsampled 


# 2 branches - objpart, semseg
# in the 1-branch method we "create" the sem.seg map from the objpart logits
class Resnet34_8s_2b(nn.Module):

    def __init__(self, fully_conv=True, pretrained=True, step_size=32,
                 semseg_num_classes=21, objpart_num_classes=2):

        super(Resnet34_8s_2b, self).__init__()
